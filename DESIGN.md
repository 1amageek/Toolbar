...
Toolbar(text:onSend:)                   ToolbarContainer {
  .toolbarMenu { ... } label: { ... }      HStack(alignment: .bottom, spacing: 8) {
  .toolbarSlashProvider(...)                 ToolbarMenuButton { ... }
  .toolbarVoiceProvider(...)                 ToolbarEditor(text: $text)
  .toolbarAccessory { ... }                  SendButton(isEnabled: ...) { ... }
...
|                Public API (SwiftUI)                        |
|                                                            |
|   ToolbarContainer { ... }.accessory { ... }.popup { ... } |
...
Toolbar/
  ...
  Sources/Toolbar/
    ToolbarContainer.swift               // public, GlassEffectContainer + slab + .popup/.accessory modifiers
...
`ToolbarContainer` の内部は **accessory + content** の 2 段構成。`.accessory { }` を付けない場合は accessory が `EmptyView` で、見た目上は content だけが slab に乗る。`popup { }` はこの slab の外側に描画される浮遊面で、slash command 候補のように composer 本体のレイアウトへ参加させたくない UI に使う。

`contentInsets` は slab の内側余白を制御する。外側の `.padding(...)` では glass 境界と content の距離を調整できないため、これは `ToolbarContainer` の API として持つ。
...
GlassEffectContainer {                  // morph domain
  VStack(alignment: .leading, spacing: 8) {
    ...
    accessory                           // ← .accessory { } で差し込まれた View
                                        //   省略時は EmptyView
                                        //   例: AttachmentChip row / VoiceWaveform / TranscribingIndicator
...
ToolbarContainer { ... }
  .popup(isPresented: text.hasPrefix("/")) {
    SlashCommandPopup(...)              // slab の外側、accessory とは別スロット
  }
```
...
- slab 内側余白は `ToolbarContainer(contentInsets:)` で制御する。toolbar 全体の外側余白は呼び出し側の `.padding(...)` で制御する
...
- Slash popup: `.glassEffect(.regular, in: .rect(cornerRadius: 12))` (`SlashCommandPopup` 内蔵)。`ToolbarContainer.popup(isPresented:)` に置き、slash 入力中だけ slab の外側の浮遊面として表示する。サイズや外側 padding は呼び出し側の `.frame(...)` / `.padding(...)` で調整する
